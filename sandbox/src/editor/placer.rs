...
use crate::{
    editor::{variated_color, CanvasDrawState},
    simulation::{world_pos_inside_canvas, Simulation},
...
impl EditorPlacer {
    ...
    pub fn update_in_place_paint_object(
        ...
        for pixel in canvas_draw_state.pixels.iter() {
            ...
            image.data[img_index * 4 + 0] = rgba[0];
...
pub fn get_object_image_files() -> Result<BTreeMap<String, Arc<BitmapImage>>> {
    ...
    let dir_path = PathBuf::from(current_dir()?.join("assets/object_images"));
