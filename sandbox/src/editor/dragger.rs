...
impl EditorDragger {
    pub fn drag_point(&self, obj_pos: Vector2<f32>, obj_angle: f32) -> Option<Vector2<f32>> {
        self.dragged_object
            .and_then(|(_o, pos)| Some(rotate_radians(pos, obj_angle) + obj_pos))
    }
    ...
    pub fn drag_object(
        ...
        if let Some(rb) = ecs_world.get::<RigidBodyHandle>(obj_id).ok() {
            ...
            self.drag_point(current_pos, rigid_body.rotation().angle())
                .and_then(|drag_pos| {
