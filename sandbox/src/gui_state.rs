...
use crate::{
    app::InputAction,
    editor::{Editor, EditorMode, EditorPlacer},
    matter::{
    ...
    simulation::{canvas_pos_to_world_pos, Simulation},
...
impl GuiState {
    ...
    pub fn add_new_matter_window(
        ...
        let reactions = self.add_matter.reactions.clone();
    ...
    pub fn add_query_tooltip(&mut self, api: &EngineApi<InputAction>, simulation: &Simulation) {
            ...
            .unwrap()
        {
            ...
            if let Some(((_rb, id), obj_pos, obj_angle)) = obj_data {
                let matter = if let Some(obj_matter) = &simulation.object_pixel_query {
                    Some(&matter_data[obj_matter.0 as usize].name)
                } else {
                    None
                };
...
fn add_matter_palette(ui: &mut Ui, simulation: &Simulation, editor: &mut Editor) {
    ...
    let mut matters: Vec<MatterDefinition> = simulation.matter_definitions.definitions.clone();
    matters.sort_unstable_by_key(|m| m.state);
    let matters: Vec<Vec<MatterDefinition>> = matters
        .group_by(|a, b| a.state == b.state)
        .map(|g| g.to_vec())
        .collect();
    let num_cols = 4;
...
fn add_object_matter_palette(ui: &mut Ui, editor: &mut Editor, matter_data: &MatterDefinitions) {
    let button_size = Vec2::new(24.0, 24.0);
    let mut matters: Vec<MatterDefinition> = matter_data
        .definitions
        .iter()
        .filter(|m| m.state == MatterState::Solid)
        .cloned()
        .collect();
    matters.sort_unstable_by_key(|m| m.state);
    let matters: Vec<Vec<MatterDefinition>> = matters
